const STORAGE_KEY = "ielts-atelier-state-v1";
      ...
      <div class="auth-card">
        ...
        <div class="auth-form">
          <div>
            ...
            <p class="section-subtitle">
              ${isLogin ? "使用邮箱和密码进入你的学习主页。" : "注册后将进入 6 步引导流程，系统会为你生成首版学习计划。"}
            </p>
          </div>
          <form data-form="${isLogin ? "login" : "register"}">
            <div class="form-grid">
              ${isLogin ? "" : `
                <div class="field">
                  <label>昵称</label>
                  <input name="nickname" placeholder="例如 Mia" required />
                </div>
              `}
            ...
            <div class="button-row" style="margin-top: 22px;">
              <button class="primary-button" type="submit">${isLogin ? "登录进入平台" : "注册并开始填写信息"}</button>
            </div>
          </form>
          <div class="auth-footer">
            当前版本暂不包含忘记密码流程，适合作为产品原型和前端交互演示。
          </div>
        </div>
      </div>
    </section>
  ...
  return `
    <div class="wizard-body">
      <div>
        ...
        <p class="section-subtitle">这里保留页面体验设置与授权默认设置。完成后系统将生成你的首版学习计划。</p>
      </div>
      ...
      <div class="wizard-card">
        <div class="wizard-meta">
          <div>
            ...
            <p class="section-subtitle">完成 6 步后，系统会先展示“你的计划已生成”页面，再带你进入首页。</p>
          </div>
          <div class="metric-card">
        ...
        <div class="button-row" style="margin-top: 30px;">
          ...
          ${step < 6 ? `<button class="primary-button" data-action="wizard-next" data-step="${step}">下一步</button>` : `<button class="primary-button" data-action="finish-onboarding">生成学习计划</button>`}
        </div>
      </div>
    </section>
  `;
    ...
    <div class="record-list">
      ...
      <section class="surface-card">
        ...
        <form data-form="settings-study" style="margin-top: 18px;">
          ...
          <div class="button-row" style="margin-top: 18px;">
            <button class="primary-button" type="submit">保存目标并重排计划</button>
          </div>
        </form>
      </section>
      <section class="surface-card">
...
function handleAuthSubmit(formData, isLogin) {
  ...
  setState((draft) => {
    ...
    draft.currentUserId = user.id;
    draft.ui.authMode = "login";
  });
  navigate("/onboarding?step=1", { force: true });
}
...
function bindGlobalEvents() {
  ...
  app.addEventListener("submit", (event) => {
    ...
    if (formType === "login") {
      handleAuthSubmit(formData, true);
      return;
    }

    if (formType === "register") {
      handleAuthSubmit(formData, false);
      return;
    }
